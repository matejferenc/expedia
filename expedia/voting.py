...
def run_solution():
    ...
    topclasters = nlargest(15, sorted(popular_hotel_cluster.items()), key=itemgetter(1))
    ...
    while 1:
        ...
        if s1 in best_hotels_od_ulc:
            ...
            topitems = nlargest(15, sorted(d.items()), key=itemgetter(1))
            for i in range(len(topitems)):
                votes[topitems[i][0]] += 1

        s2 = (srch_destination_id, hotel_country, hotel_market, is_package)
        if s2 in best_hotels_search_dest:
            d = best_hotels_search_dest[s2]
            topitems = nlargest(15, d.items(), key=itemgetter(1))
            for i in range(len(topitems)):
                votes[topitems[i][0]] += 1
        ...
        for i in range(len(topclasters)):
            votes[topclasters[i][0]] += 1
        
#         assert len(votes.items()) <= 60 & len(votes.items()) >= 5
#         print("votes length {}".format(len(votes.items())))
        print("votes")
        print(votes)
        print(votes.items())
        topvotes = nlargest(5, sorted(votes.items()), key=itemgetter(1))
        for i in range(len(topvotes)):
            out.write(' ' + topvotes[i][0])
            print(topvotes[i][0])
            if validate == 1:
                if topvotes[i][0]==hotel_cluster:
                    hits[i] +=1
